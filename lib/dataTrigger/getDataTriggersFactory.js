...
const allowOnlyOwnerTrigger = require('./allowOnlyOwnerTrigger');
const createFeatureFlagDataTrigger = require('./featureFlagTriggers/createFeatureFlagDataTrigger');
...
function getDataTriggersFactory() {
  ...
  let featureFlagDataContractId = Buffer.alloc(0);
  if (process.env.FEATURE_FLAG_CONTRACT_ID) {
    featureFlagDataContractId = Identifier.from(process.env.FEATURE_FLAG_CONTRACT_ID);
  }
